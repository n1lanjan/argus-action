...
export class GitHubService {
   ...
   */
  async postReview(review: FinalReview, context: any): Promise<void> {
    ...
    try {
      ...
      await this.postIssueComments(review, pr.number)
   ...
   */
  private async postIssueComments(review: FinalReview, prNumber: number): Promise<void> {
   ...
   */
  private async submitBlockingReview(review: FinalReview, prNumber: number, commitSha: string): Promise<void> {
   ...
   */
  private async findExistingReviewComment(prNumber: number): Promise<any> {
    ...
    return comments.data.find(comment => 
      comment.body?.includes('<!-- argus-summary -->')
    )
  }
  ...
  private formatMainReviewComment(review: FinalReview): string {
    ...
    if (review.recommendations.length > 0) {
      ...
      for (const issue of review.recommendations.slice(0, 5)) { // Limit to top 5
    ...
    if (review.coaching.length > 0 && this.config.enableCoaching) {
      ...
      for (const coaching of review.coaching.slice(0, 3)) { // Limit to top 3
    ...
    // Footer
    comment += '---\n\n'
    comment += '💬 **Speak to Argus**: Reply to specific comments or mention `@argus` for follow-up questions\n'
    comment += '🔄 **Re-summon**: Push new commits to awaken Argus for fresh review\n'
    comment += '⚙️ **Configure the Eyes**: Add `.github/argus-config.yml` to customize vigilance settings\n'
   ...
   */
  async getRepositoryStructure(path: string = ''): Promise<any[]> {
    try {
